      ...
      <Grid container spacing={{ xs: 3, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
        {data.map((x, index) => (
          <Grid item xs={2} sm={4} md={4} key={index}>
               <Card sx={{ maxWidth: 300 }} style={{cursor:"pointer"}} onClick={()=>user ? navigate("detailss/"+index) : Toastify('Please login to see details') }>
            
            <img src={x.image} alt="" style={{height:"194px", width:"100%"}} />
